...
public class ClientTrackPackage implements Initializable {
    ...
    public String addDescription(String currentStatus){
                ...
                else if(currentStatus.contains("Do Odebrania W Oddziale") || currentStatus.contains("To Be Picked In Hub")) {
    ...
    public static List<PackageHistory> translateStatuses(int id){
        ...
        List<Packages> packages = PackagesDAO.getPackagesById(statuses.get(0).getPackageId());
        ...
        if (Preference.readPreference("language").equals("english")) {
            for (int i = 0; i < statuses.size(); i++) {
                for (int j = 0; j < status.length; j++) {
                    if (statuses.get(i).getStatus().equals(status[j].displayName())) {
                        if(statuses.get(i).getStatus().equals("Do Odebrania W Oddziale")) {
                            statuses.get(i).setStatus(status[j].engDisplayName() + " - " +
                                    packages.get(0).getUsersByCourierId().getAreasByAreaId().getCity() + ", " +
                                    packages.get(0).getUsersByCourierId().getAreasByAreaId().getDepartmentStreetAndNumber());
                        }
                        else {
                            statuses.get(i).setStatus(status[j].engDisplayName());
                        }
            ...
            for (int i = 0; i < statuses.size(); i++) {
                for (int j = 0; j < status.length; j++) {
                    if (statuses.get(i).getStatus().equals(status[j].displayName())) {
                        if (statuses.get(i).getStatus().equals("Do Odebrania W Oddziale")) {
                            statuses.get(i).setStatus(status[j].displayName() + " - " +
                                    packages.get(0).getUsersByCourierId().getAreasByAreaId().getCity() + ", " +
                                    packages.get(0).getUsersByCourierId().getAreasByAreaId().getDepartmentStreetAndNumber());
                        }
                    }
