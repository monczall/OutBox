...
public class PackagesDAO {
    ...
    static public ObservableList<PackagesDTO> addTable()
    {
        ...
        String hql = "SELECT NEW main.java.entity.PackagesDTO(P.userId, P.id, P.packageNumber, P.timeOfPlannedDelivery," +
                " UI.name, UI.surname, UI.phoneNumber, UI.streetAndNumber, UI.city, PH.status, P.additionalComment) " +
                "FROM Packages P, UserInfos UI, PackageHistory PH WHERE P.id = PH.packageId AND P.userInfoId = UI.id " +
                " AND PH.status = (SELECT PH.status FROM PH WHERE PH.id = (SELECT MAX(PH.id) FROM PH WHERE PH" +
                ".packageId = P.id )) AND NOT PH.status = 'Dostarczona' AND NOT PH.status = 'Zwrócona Do Nadawcy' " +
